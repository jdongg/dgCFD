...
*/
__global__ void
computeRHScuda(int subDflg, int Nx, int Ny, int SubnElems, int nOrder2D, int nOrder1D, int nLoc, double *Q1, double *Q2, double *Q3,
	...
	for (int k = gtid; k < SubnElems; k += gridDim.x*blockDim.x) {
		...
		// extract i j where k = i*Nx + j to explicitly define neighbor cell
		// indices instead of using mapB
		int subNy = Ny/2;
		int jj = k%Nx;
		int ii = (k)/Nx;
		int nLeft = ii*Nx + ((jj+Nx-1)%Nx);
		int nRight = ii*Nx + ((jj+1)%Nx);
		int nBottom = ((ii+subNy-1)%subNy)*Nx + jj;
		int nTop = ((ii+1)%subNy)*Nx + jj;
		if ((subDflg == 0) && (ii == 0)) nBottom = k;
		if ((subDflg == 1) && (ii == subNy-1)) nTop = k;
		...
		// get top or bottom coefficients from halo if appropriate
		if ((subDflg == 0) && (ii == Ny/2-1))
		{
			for (int i = 0; i < nLoc; ++i)
			{
				...
				cTop[i] = Q1Halo[jj*nLoc+i];
				cTop[nLoc+i] = Q2Halo[jj*nLoc+i];
				cTop[2*nLoc+i] = Q3Halo[jj*nLoc+i];
				cTop[3*nLoc+i] = Q4Halo[jj*nLoc+i];
			}
		} else if ((subDflg == 1) && (k < Nx))
		{
			for (int i = 0; i < nLoc; ++i)
			{
				cLeft[i] = Q1[nLeft*nLoc+i];
				...
				cBottom[i] = Q1Halo[k*nLoc+i];
				cBottom[nLoc+i] = Q2Halo[k*nLoc+i];
				cBottom[2*nLoc+i] = Q3Halo[k*nLoc+i];	
				cBottom[3*nLoc+i] = Q4Halo[k*nLoc+i];
		...
		for (int i = 0; i < nOrder1D; ++i)
		{
			...
			for (int j = 0; j < nLoc; ++j)
			{
				...
				else if ((idx == 0) && (subDflg == 0)) { // enforce Dirichlet BCs on the bottom boundary
					Qneighbor[i] = 2.0;
					Qneighbor[nOrder1D+i] = 0.0;
					Qneighbor[2*nOrder1D+i] = 0.0;
		...
		for (int i = 0; i < nOrder1D; ++i)
		{
			...
			for (int j = 0; j < nLoc; ++j)
			{
				...
				else if ((idx == Ny/2-1) && (subDflg == 1)) { // enforce Dirichlet BCs along top edge
					Qneighbor[i] = 1.0;
					Qneighbor[nOrder1D+i] = 0.0;
					Qneighbor[2*nOrder1D+i] = 0.0;
...
__global__
void updateCUDA1(int SubnElems, int nLoc, double dt, double *Q1, double *Q2, double *Q3, double *Q4,
	             double *Q10, double *Q20, double *Q30, double *Q40, double *rhsQ1, double *rhsQ2,
	             double *rhsQ3, double *rhsQ4) {
	int gtid = threadIdx.x + blockIdx.x*blockDim.x;
	for (int i = gtid; i < SubnElems*nLoc; i += gridDim.x*blockDim.x)
	{
...
__global__
void updateCUDA2(int SubnElems, int nLoc, double dt, double *Q1, double *Q2, double *Q3, double *Q4,
	             double *Q10, double *Q20, double *Q30, double *Q40, double *rhsQ1, double *rhsQ2,
	             double *rhsQ3, double *rhsQ4) {
	int gtid = threadIdx.x + blockIdx.x*blockDim.x;
	for (int i = gtid; i < SubnElems*nLoc; i += gridDim.x*blockDim.x)
...
__global__
void updateCUDA3(int SubnElems, int nLoc, double dt, double *Q1, double *Q2, double *Q3, double *Q4,
	             double *Q10, double *Q20, double *Q30, double *Q40, double *rhsQ1, double *rhsQ2,
	             double *rhsQ3, double *rhsQ4) {
	int gtid = threadIdx.x + blockIdx.x*blockDim.x;
	for (int i = gtid; i < SubnElems*nLoc; i += gridDim.x*blockDim.x)
...
__global__
void updateCUDA4(int SubnElems, int nLoc, double *Q1, double *Q2, double *Q3, double *Q4,
